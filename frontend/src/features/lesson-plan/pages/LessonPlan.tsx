...
import {
  Calendar, Sparkles, CheckCircle2, AlertCircle,
  Loader2, Play, UploadCloud, Clock, Share2, Download,
  BookOpen, ChevronRight, Wand2, Zap, Settings, FolderOpen,
} from 'lucide-react';
import { StaggerContainer, StaggerItem } from '@/shared/components/ui/PageWrapper';
import { lessonPlanService } from '@/features/lesson-plan/services';
import { lessonPlanApi } from '@/features/lesson-plan/api';
...
export default function LessonPlan() {
  ...
  const [libraryPick, setLibraryPick] = useState<{ id: number; filename: string } | null>(null);
  ...
  const validateInputs = (): string | null => {
    if (!file && !libraryPick) return 'Please upload a syllabus file or pick one from your library.';
  ...
  const handleUpload = async () => {
    ...
    try {
      let result;
      if (libraryPick) {
        const content = await uploadedFilesApi.getContent(libraryPick.id);
        if (!content.content) throw new Error('Selected file has no extractable text.');
        result = await lessonPlanService.generatePlanFromText({
          text: content.content,
          filename: libraryPick.filename,
          startDate,
          endDate,
        ...
        result = await lessonPlanService.generatePlan({
          file: file as File,
          startDate,
          endDate,
          classesPerWeek: Number(classesPerWeek),
  ...
  const handleLibraryPick = (picked: { id: number; original_filename: string }[]) => {
    setPickerOpen(false);
    const item = picked[0];
    if (!item) return;
    setLibraryPick({ id: item.id, filename: item.original_filename });
    setFile(null);
    setErrorMsg(null);
    toast.success(`Using "${item.original_filename}" from your library.`);
  };
  ...
  const lessonDays = useMemo<LessonDay[]>(
    () => (aiAnalysis?.lesson_plan as LessonDay[] | undefined) ?? [],
    [aiAnalysis],
  );

  const pdfPayload = useMemo(
    () => ({
      lesson_plan: lessonDays,
      subject: teacherSubject || 'General',
      start_date: startDate || undefined,
      end_date: endDate || undefined,
      warning_message:
        typeof aiAnalysis?.warning_message === 'string' && aiAnalysis.warning_message
          ? aiAnalysis.warning_message
          : undefined,
      document_name:
        typeof aiAnalysis?.document_name === 'string' ? aiAnalysis.document_name : undefined,
    }),
    [aiAnalysis, lessonDays, startDate, endDate, teacherSubject],
  );

  const handleDownloadPdf = async () => {
    if (!lessonDays.length || isDownloading) return;
    setIsDownloading(true);
    try {
      const filename = `LessonPlan_${teacherSubject || 'General'}.pdf`;
      const blob = await lessonPlanApi.exportPdf({ ...pdfPayload, filename });
  ...
  const handleShareWhatsApp = async () => {
    if (!lessonDays.length || isSharing) return;
    setIsSharing(true);
    try {
      const subjectSlug = teacherSubject || 'General';
      const filename = `LessonPlan_${subjectSlug}.pdf`;
      const blob = await lessonPlanApi.exportPdf({ ...pdfPayload, filename });
      const scheduleBits =
      ...
      await shareToWhatsApp({
        blob,
        filename,
        title: `Lesson Plan — ${subjectSlug}`,
        text: `Lesson Plan — ${subjectSlug}${scheduleBits} · ${lessonDays.length} session${
          lessonDays.length === 1 ? '' : 's'
        }`,
      });
    } catch (err) {
      console.error(err);
          ...
          <div className="grid lg:grid-cols-5 gap-8 items-start">
              ...
              <div className="premium-card p-10 bg-card/40 border-glass-border relative group overflow-hidden">
                ...
                <div className="flex flex-col items-center text-center relative z-10">
                  ...
                  <p className="text-muted-foreground text-sm font-bold mb-10 max-w-[260px] leading-relaxed">
                    Upload a syllabus or chapter file (PDF, DOCX, TXT, MD) for deep <span className="text-primary font-black">Neural Mapping</span>.
                  </p>
                  ...
                  >
                    ...
                    <span className="relative z-10 truncate max-w-full px-4">
                      {file ? file.name : libraryPick ? libraryPick.filename : 'Initialize Document'}
                    </span>
                  </label>
                  <button
                    type="button"
                    onClick={() => setPickerOpen(true)}
                    className="mt-3 w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-2xl border border-white/10 bg-black/30 hover:border-primary/40 hover:bg-primary/5 text-muted-foreground hover:text-primary text-[10px] font-black uppercase tracking-[0.2em] transition-all"
                  >
                    <FolderOpen className="w-3.5 h-3.5" /> Reuse from My Files
                  </button>
                  {(file || libraryPick) && (
                    <motion.span
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="text-[10px] text-primary font-black uppercase tracking-[0.2em] mt-6 bg-primary/10 px-6 py-2 rounded-2xl border border-primary/20 aurora-glow"
                    >
                      ✓ {libraryPick ? 'From Library' : 'Target Captured'}
                    </motion.span>
                  )}
                </div>
              </div>
            ...
            <StaggerItem className="lg:col-span-3">
              <div className="premium-card p-10 bg-card/40 border-glass-border relative group h-full overflow-hidden">
                ...
                <button
                  disabled={isAnalyzing || (!file && !libraryPick)}
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-8 pb-8 border-b border-white/5">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                <button
                  ...
                  disabled={isDownloading || isSharing || !lessonDays.length}
                  className="flex-1 md:flex-none h-14 px-8 aurora-gradient text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl aurora-glow transition-all flex items-center justify-center gap-3 disabled:opacity-30"
                >
                  {isDownloading ? (
                ...
                <button
                  ...
                  disabled={isDownloading || isSharing || !lessonDays.length}
                  className="flex-1 md:flex-none h-14 px-8 bg-emerald-600/10 border border-emerald-500/30 hover:bg-emerald-600 text-emerald-500 hover:text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl transition-all flex items-center justify-center gap-3 disabled:opacity-30"
                  title="Share lesson plan PDF to WhatsApp"
                >
                  {isSharing ? (
                    <Loader2 className="w-5 h-5 animate-spin" />
                  ) : (
