...
function AttachmentPreview({ url }: { url: string }) {
  ...
  if (type === 'image') {
    return (
      <a href={fullUrl} target="_blank" rel="noopener noreferrer" className="block mt-4 rounded-2xl overflow-hidden border border-white/10 hover:border-primary/40 transition-all">
        <img src={fullUrl} alt="Attachment" className="w-full max-h-64 object-cover" />
      </a>
    );
  }

  const icons = { pdf: FileText, doc: File, video: Film, other: Paperclip, image: ImageIcon };
  const labels = { pdf: 'PDF Document', doc: 'Document', video: 'Video', other: 'File', image: 'Image' };
  ...
  return (
    <a
      ...
      className="mt-4 flex items-center gap-4 p-5 rounded-2xl bg-primary/10 border border-primary/20 hover:bg-primary/20 transition-all group"
    >
      ...
      <div>
        <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-60">{labels[type]}</p>
        <p className="text-sm font-bold text-foreground italic">Open Attachment</p>
      </div>
      <ChevronRight className="w-5 h-5 text-primary ml-auto group-hover:translate-x-1 transition-all" />
    </a>
  );
}
...
export default function ParentAnnouncements() {
  ...
  const handleMarkAsRead = async (a: Announcement) => {
    // Mark optimistically in UI — backend mark-as-read is best-effort for parent
        ...
        {selected && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-white rounded-[3rem] shadow-2xl border border-white/50 overflow-hidden flex flex-col max-h-[90vh]"
            >
              {/* Priority bar */}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-2 text-xs text-slate-500 font-medium flex-wrap">
                      ...
                      <span className={cn('px-2 py-0.5 rounded-full text-[10px] font-black uppercase', PRIORITY_STYLES[selected.priority as keyof typeof PRIORITY_STYLES]?.badge)}>
                        {selected.priority}
                      </span>
                    </div>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto px-8 py-8 space-y-6">
                ...
                {selected.attachment_url && (
                  <AttachmentPreview url={selected.attachment_url} />
                )}
